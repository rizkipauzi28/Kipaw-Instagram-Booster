...
export const MobileNav: React.FC<MobileNavProps> = ({
  ...
  return (
    <div className="md:hidden fixed bottom-3 left-3 right-3 z-40">
      <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800/80 shadow-2xl rounded-2xl px-2 py-1.5">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (!currentUser && tab.id !== 'tasks' && tab.id !== 'dashboard') {
                    onOpenAuth();
                  } else {
                    setActiveTab(tab.id);
                  }
